...
async function getClient(webhookId: string) {
  if (
    !(await redis.get(`webhooksSeen:${webhookId}`)) ||
    (await redis.get(`webhooksSeen:${webhookId}`)) === "false"
  ) {
    return clientSafe();
  } else {
    return client();
...
const webhookInvalidPostRatelimit = slowDown({
  ...
  skip(req, res) {
    return !(
      ...
      res.statusCode < 500 &&
      res.statusCode !== 429
    ); // trigger if it's a 4xx but not a ratelimit
  },
...
async function preRequestChecks(req: Request, res: Response, gameId?: string) {
  ...
  if (ratelimit === 0) {
    ...
    const ttl =
      ...
      (await redis.ttl(`webhookRatelimit:${req.params.id}`));
    ...
    res.status(429).json({
      proxy: true,
      message:
        "You have been ratelimited. Please respect the standard Discord ratelimits.",
    });
    return false;
  }
  ...
  async (req, res) => {
    ...
    if (!axios) {
      res.status(403).json({
        proxy: true,
        error:
          "The proxy has not seen your webhook before, and is currently unable to service your request.",
      });
      return false;
    }

    const response = await axios[0].post(
      `https://discord.com/api/webhooks/${req.params.id}/${
        req.params.token
      }?wait=${wait}${threadId ? "&thread_id=" + threadId : ""}`,
      body,
      {
        headers: {
...
app.post(
  ...
  async (req, res) => {
    if (!config.queue.enabled)
      return res
        .status(403)
        .json({ proxy: true, error: "Queues have been disabled." });
